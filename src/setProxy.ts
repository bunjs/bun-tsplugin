...
export = (languageService: tss.LanguageService, proxy: any, extraLength: any, info: any) => {
    proxy.getCompilerOptionsDiagnostics = () => {
        const prior = languageService.getCompilerOptionsDiagnostics();
        return prior.map(diagnostic => {
            diagnostic.start = (diagnostic.start as number) - extraLength;
            return diagnostic;
        });
    }
    ...
    proxy.getEncodedSemanticClassifications = (fileName: string, span: tss.TextSpan) => {
        span.start = span.start - extraLength;
        const prior = languageService.getEncodedSemanticClassifications(fileName, span);
        return prior;
    }
    proxy.getEncodedSyntacticClassifications = (fileName: string, span: tss.TextSpan) => {
        span.start = span.start - extraLength;
        const prior = languageService.getEncodedSemanticClassifications(fileName, span);
        return prior;
    }
    proxy.getFormattingEditsAfterKeystroke = (fileName: string, position: number, key: string, options: tss.FormatCodeOptions | tss.FormatCodeSettings) => {
        position = position - extraLength;
        const prior = languageService.getFormattingEditsAfterKeystroke(fileName, position, key, options);
        return prior;
    }
    
    proxy.getIndentationAtPosition = (fileName: string, position: number, options: tss.EditorOptions | tss.EditorSettings) => {
        position = position - extraLength;
        const prior = languageService.getIndentationAtPosition(fileName, position, options);
        return prior;
    }
    proxy.getNameOrDottedNameSpan = (fileName: string, startPos: number, endPos: number) => {
        ...
        (prior as tss.TextSpan).start = (prior?.start as number) - extraLength;
        return prior;
    }
    ...
    proxy.getSemanticDiagnostics = (fileName: string) => {
        ...
        prior = prior.filter((diagnostic) => {
            if (diagnostic.code == 2448) {
                ...
                //     diagnostic.start!
                // );
                if ((diagnostic.messageText as tss.DiagnosticMessageChain).messageText) {
                    return (diagnostic.messageText as tss.DiagnosticMessageChain).messageText.indexOf("App' used before its declaration") === -1;
                } else {
                    return (diagnostic.messageText as string).indexOf("App' used before its declaration") === -1;
                }
            }
            return true;
        });
    ...
    proxy.getCompletionsAtPosition = (fileName: string, position: number, options: tss.GetCompletionsAtPositionOptions | undefined) => {
        position = position + extraLength;
        ...
        (prior as any).entries = prior?.entries.map(e => {
